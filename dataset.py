...
class SiameseWhaleDataset(datasets.ImageFolder):

    def __init__(self, dir, n_batch, n_cls, n_same, transform=None, *arg, **kw):
        super(SiameseWhaleDataset, self).__init__(dir,transform)

        self.n_batch = n_batch
        self.n_cls = n_cls
        self.n_same = n_same

        self.pairs = self.generate_pairs(self.imgs, self.n_batch, self.n_cls, self.n_same, len(self.classes))

    @staticmethod
    def generate_pairs(imgs, n_batch, n_cls, n_same,n_classes):
        def create_indices(_imgs):
            inds = dict()
            for idx, (img_path,label) in enumerate(_imgs):
                if label not in inds:
        ...
        for b in tqdm(range(n_batch)):
            ...
            for cls in range(n_cls):
                ...
                while already_sepected[c] == 1:
                    c = np.random.randint(0, n_classes - 1)
                already_sepected[c] = 1
                for s in range(n_same):
                    if len(indices[c]) == 1:
                        n1, n2 = 0, 0
                    elif len(indices[c]) == 2:
                        n1, n2 = 0, 1
                    else:
                        n1 = np.random.randint(0, len(indices[c]) - 1)
                        n2 = np.random.randint(0, len(indices[c]) - 1)
                    pairs.append([indices[c][n1], indices[c][n2], c])
    ...
    def __getitem__(self, index):
        ...
        # Get the index of each image in the triplet
        a, p, c = self.pairs[index]

        # transform images if required
        img_a, img_p = transform(a), transform(p)
        return img_a, img_p, c
